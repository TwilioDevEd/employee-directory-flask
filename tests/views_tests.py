from employee_directory_flask import app
...
class RootTest(BaseTest):
    def test_no_results(self):
        response = self.client.post(
            '/directory/search', data={'Body': 'you_will_not_find_me'}
        )
    ...
    def test_single_result(self):
        response = self.client.post('/directory/search', data={'Body': 'Wolverine'})
        self.assertEquals(200, response.status_code)
    ...
    def test_multiple_results_list_on_message_body(self):
        response = self.client.post('/directory/search', data={'Body': 'Thor'})
        self.assertEquals(200, response.status_code)
    ...
    def test_multiple_results_store_names_on_session(self):
        with app.test_client() as client:
            client.post('/directory/search', data={'Body': 'Thor'})
    ...
    def test_user_chooses_an_option(self):
        with app.test_client() as client:
            ...
            response = client.post('/directory/search', data={'Body': '1'})
    ...
    def test_invalid_option_triggers_a_search(self):
        with app.test_client() as client:
            with client.session_transaction() as session:
                session['choices'] = ['Thor Girl', 'Frog Thor', 'Thor']
            response = client.post('/directory/search', data={'Body': '51'})
        self.assertEquals(200, response.status_code)
